...
int osa_test_thread8(void);
static OSA_RESULT _thread8_proc(OSA_T_ARGS arg);
...
int osa_test_thread8(void)
{
	...
	res = osa_t_create(&hThread8, _thread8_proc, 0, 16*1024, 0);
	if( 0 != res ) {
...
OSA_RESULT _thread8_proc(OSA_T_ARGS arg)
{
	test_db("[MSG 1] Thread8 running\n");
