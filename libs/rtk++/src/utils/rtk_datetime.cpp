...
RDateTime::RDateTime(DateTime_Type dtt)
{
    timeZone = g_timeZone;
...
RDateTime::RDateTime(ri32 y, ri32 m, ri32 d)
{
    time_t rawtime;
    struct tm *timeinfo;
...
RDateTime::RDateTime(ri32 h, ri32 min_, ri32 s, ri32 ns)
{
    time_t rawtime;
    struct tm *timeinfo;
    struct timeval tm_val;
...
int RDateTime::setDateTime(const RDateTime &dt)
{
    year    = dt.year;
    month   = dt.month;
    day     = dt.day;
...
int RDateTime::setDate(ri32 y, ri32 m, ri32 d,
            DateTime_Type dtt)
{
    year        = y;
    month       = m;
...
int RDateTime::setTime(ri32 h, ri32 min_, ri32 s, ri32 ns)
{
    hour        = h;
    min         = min_;
    sec         = s;
...
int RDateTime::setTime(ri32 h, ri32 min_, double fs)
{
    hour        = h;
    min         = min_;
    sec         = (ri32)(fs);
...
int RDateTime::setDateTime(ri32 y, ri32 m, ri32 d,
                          ri32 h, ri32 min_, ri32 s,
                          ri32 ns,
                          DateTime_Type dtt)
{
...
int RDateTime::setCurrentDate(DateTime_Type dtt)
{
    time_t rawtime;
    struct tm *timeinfo;
    struct timeval tm_val;
...
int RDateTime::setCurrentTime(DateTime_Type dtt)
{
    time_t rawtime;
    struct tm *timeinfo;
    struct timeval tm_val;
...
int RDateTime::setCurrentDateTime(DateTime_Type dtt)
{
    time_t rawtime;
    struct tm *timeinfo;
    struct timeval tm_val;
...
int RDateTime::timeFromString(const char *fmt, const char *sTime)
{
    // FIXME: not implemented
    return 0;
}

int RDateTime::dateFromString(const char *fmt, const char *sDate)
{
    // FIXME: not implemented
    return 0;
}

int RDateTime::dateTimeFromString(const char *fmt, const char *sDateTime)
{
    // FIXME: not implemented
    return 0;
}
...
const int RDateTime::toTM(struct tm *t) const
{
    t->tm_year = year  - 1900;
    t->tm_mon  = month - 1;
    t->tm_mday = day;
...
int RDateTime::fromTime_t(ri64 t)
{
    time_t      tt;
    struct tm   *tm1;
...
int RDateTime::fromTimeStamp(ri64 ts)
{
    time_t      t;
    struct tm   *tm1;
...
std::ostream& operator << (std::ostream &os, const RDateTime &dt)
{
    char    c1, c2;

    if( dt.dt_type == DATETIME_LOCAL ) c1 = 'L';
    else                               c1 = 'U';
    if( dt.timeZone >= 0 ) c2 = '+';
    else                   c2 = '-';
...
int RDateTime::toStream(RDataStream &s, int isSingleDS)
{
    ri32    dt;
